...
- status: resolved
## 2026-05-27
- time: 2026-05-27 10:20 KST
- location: verification -> `npm install framer-motion`
- summary: PowerShell execution policy blocked the npm wrapper
- details: `npm install framer-motion` failed because `npm.ps1` is blocked by the local execution policy. Use `npm.cmd` or `cmd /c npm install framer-motion` instead.
- status: open
## 2026-05-27
- time: 2026-05-27 10:21 KST
- location: verification -> `cmd /c npm install framer-motion`
- summary: package installation succeeded after PowerShell fallback
- details: `framer-motion` was added successfully through `cmd /c npm install framer-motion` after the PowerShell wrapper failure.
- status: resolved
## 2026-05-27
- time: 10:47 KST
- location: `npm run build`
- summary: PowerShell execution policy blocked npm script launch
- details: `npm.ps1` could not run because script execution is disabled in this shell. The build check will be rerun through `cmd /c npm run build`.
- status: resolved
## 2026-05-27
- time: 10:55 KST
- location: `git add ... && git commit ...`
- summary: PowerShell rejected chained git commands
- details: The initial commit command failed because PowerShell does not accept `&&` as a statement separator in this session. The commit will be rerun through `cmd /c`.
- status: resolved
